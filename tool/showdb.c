...
static void decodeCell(
  ...
  if( cType!=5 ){
    ...
    if( i<0 ){ printf("Error: Integer overflow in payload size\n"); return; }
    printBytes(a, x, i);
    nLocal = localPayload(nPayload, cType);
    if( nLocal==nPayload ){
  ...
  if( cType==5 || cType==13 ){
    ...
    if( i<0 ){ printf("Error: Integer overflow in rowid\n"); return; }
    printBytes(a, x, i);
    printf("rowid: %lld\n", rowid);
    x += i;
  }
  if( nLocal>0 ){
    ...
    if( i<0 ){ printf("Error: Integer overflow in header size\n"); return; }
    printBytes(a, x, i);
    ...
    while( x+j<=end && j<nHdr ){
       ...
       if( i<0 ){ printf("Error: Integer overflow in type code\n"); return; }
       printBytes(a, x+j, i);
...
static void page_usage_cell(
  ...
  if( cType!=5 ){
    i = decodeVarint(a, &nPayload);
    if( i<0 ) return; /* Handle overflow error silently */
    a += i;
    n += i;
    nLocal = localPayload(nPayload, cType);
  }else{
